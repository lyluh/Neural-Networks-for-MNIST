...
class LinearLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if generator:
            self.weight = torch.nn.Parameter(torch.randn(dim_in, dim_out, generator=generator, dtype=torch.float))
            self.bias = torch.nn.Parameter(torch.randn(dim_out, generator=generator, dtype=torch.float))
        else:
            self.weight = torch.nn.Parameter(torch.randn(dim_in, dim_out, dtype=torch.float))
            self.bias = torch.nn.Parameter(torch.randn(dim_out, dtype=torch.float))
